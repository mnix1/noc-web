...
(function () {
    ...
    Object.assign(FBXLoader.prototype, {
        ...
        load: function (url, onLoad, onProgress, onError) {
            ...
            let resourceDirectory = THREE.LoaderUtils.extractUrlBase(url);
            ...
            loader.load(url, function (buffer) {
                ...
                try {
                    ...
                    let scene = self.parse(buffer, resourceDirectory);
                    onLoad(scene);
        ...
        parse: function (FBXBuffer, resourceDirectory) {
            ...
            let textures = parseTextures(FBXTree, new THREE.TextureLoader(this.manager).setPath(resourceDirectory), images, connections);
            let materials = parseMaterials(FBXTree, textures, connections);
            let deformers = parseDeformers(FBXTree, connections);
            let geometryMap = parseGeometries(FBXTree, connections, deformers);
            let sceneGraph = parseScene(FBXTree, connections, deformers.skeletons, geometryMap, materials);
    ...
    function parseConnections(FBXTree) {
        ...
        if ('Connections' in FBXTree) {
            ...
            let rawConnections = FBXTree.Connections.connections;

            rawConnections.forEach(function (rawConnection) {

                let fromID = rawConnection[0];
                let toID = rawConnection[1];
                let relationship = rawConnection[2];
    ...
    function parseSkeleton(connections, deformerNodes) {
        ...
        connections.children.forEach(function (child) {
            ...
            let boneNode = deformerNodes[child.ID];
    ...
    function parseMorphTargets(relationships, deformerNode, deformerNodes, connections) {
        ...
        for (let i = 0; i < relationships.children.length; i++) {
            ...
            let child = relationships.children[i];

            let morphTargetNode = deformerNodes[child.ID];
    ...
    function parseGeometries(FBXTree, connections, deformers) {
        ...
        if ('Geometry' in FBXTree.Objects) {
            ...
            let geoNodes = FBXTree.Objects.Geometry;
    ...
    function parseMeshGeometry(FBXTree, relationships, geoNode, deformers) {
        ...
        let skeletons = deformers.skeletons;
        let morphTargets = deformers.morphTargets;

        let modelNodes = relationships.parents.map(function (parent) {
        ...
        let skeleton = relationships.children.reduce(function (skeleton, child) {
        ...
        let morphTarget = relationships.children.reduce(function (morphTarget, child) {
        ...
        let preTransform = new THREE.Matrix4();
        ...
        let modelNode = modelNodes[0];

        if ('GeometricRotation' in modelNode) {

            let array = modelNode.GeometricRotation.value.map(THREE.Math.degToRad);
            array[3] = 'ZYX';
    ...
    function genGeometry(FBXTree, geoNode, skeleton, morphTarget, preTransform) {
        ...
        let geoInfo = getGeoInfo(geoNode, skeleton);

        let buffers = genBuffers(geoInfo);

        let positionAttribute = new THREE.Float32BufferAttribute(buffers.vertex, 3);
        ...
        if (buffers.normal.length > 0) {
            ...
            let normalAttribute = new THREE.Float32BufferAttribute(buffers.normal, 3);

            let normalMatrix = new THREE.Matrix3().getNormalMatrix(preTransform);
        ...
        if (geoInfo.material && geoInfo.material.mappingType !== 'AllSame') {
            ...
            if (geo.groups.length > 0) {
                ...
                let lastGroup = geo.groups[geo.groups.length - 1];
                let lastIndex = lastGroup.start + lastGroup.count;
    ...
    function genBuffers(geoInfo) {
        ...
        geoInfo.vertexIndices.forEach(function (vertexIndex, polygonVertexIndex) {
            ...
            if (geoInfo.color) {
                ...
                let data = getData(polygonVertexIndex, polygonIndex, vertexIndex, geoInfo.color);
                    ...
                    weights.forEach(function (weight, weightIndex) {
                        ...
                        Weight.forEach(function (comparedWeight, comparedWeightIndex, comparedWeightArray) {
                            ...
                            if (currentWeight > comparedWeight) {
                                ...
                                let tmp = wIndex[comparedWeightIndex];
            ...
            if (geoInfo.normal) {
                ...
                let data = getData(polygonVertexIndex, polygonIndex, vertexIndex, geoInfo.normal);
            ...
            if (geoInfo.uv) {
                ...
                geoInfo.uv.forEach(function (uv, i) {
                    ...
                    let data = getData(polygonVertexIndex, polygonIndex, vertexIndex, uv);
    ...
    function addMorphTargets(FBXTree, parentGeo, parentGeoNode, morphTarget, preTransform) {
        ...
        morphTarget.rawTargets.forEach(function (rawTarget) {
            ...
            let morphGeoNode = FBXTree.Objects.Geometry[rawTarget.geoID];
    ...
    function genMorphGeometry(parentGeo, parentGeoNode, morphGeoNode, preTransform) {
        ...
        let vertexIndices = (parentGeoNode.PolygonVertexIndex !== undefined) ? parentGeoNode.PolygonVertexIndex.a : [];

        // make a copy of the parent's vertex positions
        let vertexPositions = (parentGeoNode.Vertices !== undefined) ? parentGeoNode.Vertices.a.slice() : [];

        let morphPositions = (morphGeoNode.Vertices !== undefined) ? morphGeoNode.Vertices.a : [];
        let indices = (morphGeoNode.Indexes !== undefined) ? morphGeoNode.Indexes.a : [];

        for (let i = 0; i < indices.length; i++) {

            let morphIndex = indices[i] * 3;
        ...
        let morphBuffers = genBuffers(morphGeoInfo);

        let positionAttribute = new THREE.Float32BufferAttribute(morphBuffers.vertex, 3);
    ...
    function getNormals(NormalNode) {
        ...
        let mappingType = NormalNode.MappingInformationType;
        let referenceType = NormalNode.ReferenceInformationType;
        let buffer = NormalNode.Normals.a;
        let indexBuffer = [];
    ...
    function getUVs(UVNode) {
        ...
        let mappingType = UVNode.MappingInformationType;
        let referenceType = UVNode.ReferenceInformationType;
        let buffer = UVNode.UV.a;
        let indexBuffer = [];
        if (referenceType === 'IndexToDirect') {
    ...
    function getColors(ColorNode) {
        ...
        let mappingType = ColorNode.MappingInformationType;
        let referenceType = ColorNode.ReferenceInformationType;
        let buffer = ColorNode.Colors.a;
        let indexBuffer = [];
        if (referenceType === 'IndexToDirect') {
    ...
    function getMaterials(MaterialNode) {
        ...
        let mappingType = MaterialNode.MappingInformationType;
        let referenceType = MaterialNode.ReferenceInformationType;
        ...
        let materialIndexBuffer = MaterialNode.Materials.a;
    ...
    function getData(polygonVertexIndex, polygonIndex, vertexIndex, infoObject) {
        ...
        let from = index * infoObject.dataSize;
        let to = from + infoObject.dataSize;
    ...
    function parseNurbsGeometry(geoNode) {
        ...
        // let order = parseInt(geoNode.Order);
        //
        // if (isNaN(order)) {
        //
        ...
        // let degree = order - 1;
        //
        // let knots = geoNode.KnotVector.a;
        // let controlPoints = [];
        // let pointsValues = geoNode.Points.a;
        //
        ...
        // }
        //
        // let curve = new THREE.NURBSCurve(degree, knots, controlPoints, startKnot, endKnot);
    ...
    function parseScene(FBXTree, connections, skeletons, geometryMap, materialMap) {
        ...
        let sceneGraph = new THREE.Group();

        let modelMap = parseModels(FBXTree, skeletons, geometryMap, materialMap, connections);

        let modelNodes = FBXTree.Objects.Model;

        modelMap.forEach(function (model) {

            let modelNode = modelNodes[model.ID];
            setLookAtProperties(FBXTree, model, modelNode, connections, sceneGraph);

            let parentConnections = connections.get(model.ID).parents;

            parentConnections.forEach(function (connection) {

                let parent = modelMap.get(connection.ID);
    ...
    function createCamera(FBXTree, relationships) {
        ...
        relationships.children.forEach(function (child) {
            ...
            let attr = FBXTree.Objects.NodeAttribute[child.ID];
        ...
        if (cameraAttribute === undefined) {
            ...
            let focalLength = cameraAttribute.FocalLength ? cameraAttribute.FocalLength.value : null;
    ...
    function createCurve(relationships, geometryMap) {
        ...
        let geometry = relationships.children.reduce(function (geo, child) {
        ...
        let material = new THREE.LineBasicMaterial({color: 0x3300ff, linewidth: 1});
    ...
    function createAmbientLight(FBXTree, sceneGraph) {
        ...
        if ('GlobalSettings' in FBXTree && 'AmbientColor' in FBXTree.GlobalSettings) {
            ...
            let ambientColor = FBXTree.GlobalSettings.AmbientColor.value;
            let r = ambientColor[0];
            let g = ambientColor[1];
            let b = ambientColor[2];
    ...
    function setLookAtProperties(FBXTree, model, modelNode, connections, sceneGraph) {
        ...
        if ('LookAtProperty' in modelNode) {
            ...
            let children = connections.get(model.ID).children;

            children.forEach(function (child) {

                if (child.relationship === 'LookAtProperty') {

                    let lookAtTarget = FBXTree.Objects.Model[child.ID];
    ...
    function setModelTransforms(FBXTree, model, modelNode) {
        ...
        if ('RotationOrder' in modelNode) {
            ...
            let value = parseInt(modelNode.RotationOrder.value, 10);
        ...
        if ('Lcl_Rotation' in modelNode) {
            ...
            let rotation = modelNode.Lcl_Rotation.value.map(THREE.Math.degToRad);
            rotation.push('ZYX');
        ...
        if ('PreRotation' in modelNode) {
            ...
            let array = modelNode.PreRotation.value.map(THREE.Math.degToRad);
            array[3] = 'ZYX';
    ...
    function bindSkeleton(FBXTree, skeletons, geometryMap, modelMap, connections) {
        ...
        let bindMatrices = parsePoseNodes(FBXTree);
        ...
        for (let ID in skeletons) {
            ...
            parents.forEach(function (parent) {
                ...
                if (geometryMap.has(parent.ID)) {
                    ...
                    let geoID = parent.ID;
                    let geoRelationships = connections.get(geoID);

                    geoRelationships.parents.forEach(function (geoConnParent) {

                        if (modelMap.has(geoConnParent.ID)) {

                            let model = modelMap.get(geoConnParent.ID);
    ...
    function parsePoseNodes(FBXTree) {
        ...
        if ('Pose' in FBXTree.Objects) {
            ...
            let BindPoseNode = FBXTree.Objects.Pose;

            for (let nodeID in BindPoseNode) {

                if (BindPoseNode[nodeID].attrType === 'BindPose') {

                    let poseNodes = BindPoseNode[nodeID].PoseNode;
    ...
    function parseAnimations(FBXTree, connections) {
        ...
        let curveNodesMap = parseAnimationCurveNodes(FBXTree);

        parseAnimationCurves(FBXTree, connections, curveNodesMap);

        let layersMap = parseAnimationLayers(FBXTree, connections, curveNodesMap);
        let rawClips = parseAnimStacks(FBXTree, connections, layersMap);
    ...
    function parseAnimationCurveNodes(FBXTree) {
        ...
        let rawCurveNodes = FBXTree.Objects.AnimationCurveNode;

        let curveNodesMap = new Map();

        for (let nodeID in rawCurveNodes) {

            let rawCurveNode = rawCurveNodes[nodeID];
    ...
    function parseAnimationCurves(FBXTree, connections, curveNodesMap) {
        ...
        let rawCurves = FBXTree.Objects.AnimationCurve;
        ...
        for (let nodeID in rawCurves) {
            ...
            let relationships = connections.get(animationCurve.id);

            if (relationships !== undefined) {

                let animationCurveID = relationships.parents[0].ID;
                let animationCurveRelationship = relationships.parents[0].relationship;
    ...
    function parseAnimationLayers(FBXTree, connections, curveNodesMap) {
        ...
        let rawLayers = FBXTree.Objects.AnimationLayer;
            ...
            if (connection !== undefined) {
                ...
                children.forEach(function (child, i) {
                    ...
                    if (curveNodesMap.has(child.ID)) {
                        ...
                        let curveNode = curveNodesMap.get(child.ID);
                        ...
                        if (curveNode.curves.x !== undefined || curveNode.curves.y !== undefined || curveNode.curves.z !== undefined) {
                            ...
                            if (layerCurveNodes[i] === undefined) {
                                ...
                                let rawModel = FBXTree.Objects.Model[modelID.toString()];
    ...
    function parseAnimStacks(FBXTree, connections, layersMap) {
        ...
        let rawStacks = FBXTree.Objects.AnimationStack;
        ...
        for (let nodeID in rawStacks) {
            ...
            let layer = layersMap.get(children[0].ID);
    ...
    function addAnimations(FBXTree, connections, sceneGraph) {
        ...
        let rawClips = parseAnimations(FBXTree, connections);
        ...
        for (let key in rawClips) {
            ...
            let rawClip = rawClips[key];

            let clip = addClip(rawClip);
    ...
    function generateTracks(rawTracks) {
        ...
        if (rawTracks.T !== undefined && Object.keys(rawTracks.T.curves).length > 0) {
            ...
            let positionTrack = generateVectorTrack(rawTracks.modelName, rawTracks.T.curves, rawTracks.initialPosition, 'position');
        ...
        if (rawTracks.R !== undefined && Object.keys(rawTracks.R.curves).length > 0) {
            ...
            let rotationTrack = generateRotationTrack(rawTracks.modelName, rawTracks.R.curves, rawTracks.initialRotation, rawTracks.preRotations);
        ...
        if (rawTracks.S !== undefined && Object.keys(rawTracks.S.curves).length > 0) {
            ...
            let scaleTrack = generateVectorTrack(rawTracks.modelName, rawTracks.S.curves, rawTracks.initialScale, 'scale');
    ...
    function generateVectorTrack(modelName, curves, initialValue, type) {
        ...
        let times = getTimesForAllAxes(curves);
        let values = getKeyframeTrackValues(times, curves, initialValue);
    ...
    function getKeyframeTrackValues(times, curves, initialValue) {
        ...
        times.forEach(function (time) {
            ...
            if (xIndex !== -1) {
                ...
                let xValue = curves.x.values[xIndex];
                values.push(xValue);
                prevValue[0] = xValue;
            ...
            if (yIndex !== -1) {
                ...
                let yValue = curves.y.values[yIndex];
                values.push(yValue);
                prevValue[1] = yValue;
            ...
            if (zIndex !== -1) {
                ...
                let zValue = curves.z.values[zIndex];
                values.push(zValue);
                prevValue[2] = zValue;
    ...
    function interpolateRotations(curve) {
        ...
        for (let i = 1; i < curve.values.length; i++) {
            ...
            let initialValue = curve.values[i - 1];
            let valuesSpan = curve.values[i] - initialValue;

            let absoluteSpan = Math.abs(valuesSpan);

            if (absoluteSpan >= 180) {

                let numSubIntervals = absoluteSpan / 180;

                let step = valuesSpan / numSubIntervals;
                let nextValue = initialValue + step;

                let initialTime = curve.times[i - 1];
                let timeSpan = curve.times[i] - initialTime;
                let interval = timeSpan / numSubIntervals;
    ...
    Object.assign(TextParser.prototype, {
        ...
        parse: function (text) {
            ...
            split.forEach(function (line, i) {
                ...
                let matchComment = line.match(/^[\s\t]*;/);
                let matchEmpty = line.match(/^[\s\t]*$/);

                if (matchComment || matchEmpty) return;

                let matchBeginning = line.match('^\\t{' + self.currentIndent + '}(\\w+):(.*){', '');
                let matchProperty = line.match('^\\t{' + (self.currentIndent) + '}(\\w+):[\\s\\t\\r\\n](.*)');
                let matchEnd = line.match('^\\t{' + (self.currentIndent - 1) + '}}');
        ...
        parseNodeBegin: function (line, property) {
            ...
            let nodeName = property[1].trim().replace(/^"/, '').replace(/"$/, '');

            let nodeAttrs = property[2].split(',').map(function (attr) {
            ...
            let attrs = this.parseNodeAttr(nodeAttrs);
        ...
        parseNodeProperty: function (line, property, contentLine) {
            ...
            let currentNode = this.getCurrentNode();
            let parentName = currentNode.name;
        ...
        parseNodeSpecialProperty: function (line, propName, propValue) {
            ...
            let innerPropName = props[0];
            let innerPropType1 = props[1];
            let innerPropType2 = props[2];
            let innerPropFlag = props[3];
    ...
    Object.assign(BinaryParser.prototype, {
        ...
        parseNode: function (reader, version) {
            ...
            while (endOffset > reader.getOffset()) {
                ...
                let subNode = this.parseNode(reader, version);
        ...
        parseSubNode: function (name, node, subNode) {
            ...
            if (subNode.singleProperty === true) {
                ...
                let value = subNode.propertyList[0];
    ...
    function isFbxFormatBinary(buffer) {
        ...
        let CORRECT = 'Kaydara FBX Binary  \0';
    ...
    function isFbxFormatASCII(text) {
        ...
        let CORRECT = ['K', 'a', 'y', 'd', 'a', 'r', 'a', '\\', 'F', 'B', 'X', '\\', 'B', 'i', 'n', 'a', 'r', 'y', '\\', '\\'];

        let cursor = 0;

        function read(offset) {

            let result = text[offset - 1];
    ...
    function getFbxVersion(text) {
        ...
        let versionRegExp = /FBXVersion: (\d+)/;
    ...
    function parseNumberArray(value) {
        ...
        let array = value.split(',').map(function (val) {
